...
class lig:
    def __init__(self, smiles, regular_name, molecule, charge, di, own, own_path):
        ...
        if own:
            ...
            AllChem.EmbedMolecule(b, randomSeed=0xf00d)
    ...
    def PDBMAKER(self, name, smiles):
        ...
        with open(
            ...
            for lines in line_to_print:
                ...
                new.write("\n")
            subprocess.run([
                               f"rm {os.path.join(self.dir, self.mol, f'{self.mol}.pdb')} && mv {os.path.join(self.dir, self.mol, 'new.pdb')} {os.path.join(self.dir, self.mol, f'{self.mol}.pdb')}"],
                           shell=True, check=True)

    def own(self, smiles, regular_name, molecule, charge, direc, own, own_path):
